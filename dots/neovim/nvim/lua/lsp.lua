...
vim.api.nvim_create_autocmd("LspAttach", {
	callback = function(args)
		...
		if client and client:supports_method(vim.lsp.protocol.Methods.textDocument_documentHighlight, bufnr) then
			local highlight_augroup = vim.api.nvim_create_augroup("myConfig/lsp_highlight", { clear = false })
			...
			vim.api.nvim_create_autocmd("LspDetach", {
				callback = function(detach_args)
					...
					vim.api.nvim_clear_autocmds({
						group = "myConfig/lsp_highlight",
						buffer = detach_args.buf,
					})
				end,
				group = vim.api.nvim_create_augroup("myConfig/lsp_detach", { clear = true }),
			})
		end
	end,
	group = vim.api.nvim_create_augroup("myConfig/lsp_attach", { clear = true }),
})
